function fish_right_prompt
    ...
    # Fast-path for Google Drive FUSE mount:
    # 1. Skip non-project directories immediately (e.g. MacObsidian, notes) to avoid FUSE parent-tree walks.
    # 2. Flag Google Drive paths so we can skip slow full-tree git scans (status --porcelain).
    set -l is_gdrive 0
    set -l real_pwd (pwd -P 2>/dev/null; or echo "$PWD")
    if string match -q "*/google-drive*" "$PWD"; or string match -q "*/mnt/google-drive*" "$real_pwd"
        set is_gdrive 1
        if not test -e ".git"; and not string match -q "*/Proyectos/*" "$PWD"
            set_color --reset
            return
        end
    end
    ...
    # On Google Drive FUSE mounts, show the branch and commit, but skip heavy tree-wide scans
    # (rev-list and git status --porcelain) to eliminate prompt latency and prevent terminal freezing.
    if test $is_gdrive -eq 1
        set_color -o
        if test -n "$branch"
